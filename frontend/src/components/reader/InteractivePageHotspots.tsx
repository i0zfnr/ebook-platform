...
import { HelpCircle, Play, Gamepad2, ArrowRight } from 'lucide-react';
...
export const InteractivePageHotspots: React.FC<InteractivePageHotspotsProps> = ({
  ...
  const currentElements = interactiveElements.filter(
    (el) => el.type !== 'qr_link' && (el.pageNumber === currentPage || el.pageNumber === currentPage + 1)
  );
      ...
      {currentElements.map((el) => {
        ...
        return (
          ...
          >
            ...
            {isFlashcards && <Gamepad2 className="h-4 w-4" />}

            <div className="flex flex-col text-left">
              <span className="text-[10px] uppercase font-mono-code font-bold opacity-90">
                Page {el.pageNumber} • {isFlashcards ? 'Speed Match Game' : el.type}
              </span>
